...
export class DetailBlogComponent implements OnInit {
      ...
      .then((result) => {
        ...
        if (result.isConfirmed) {
          ...
          this.commentaireService.deleteCommentaire(id).subscribe(
            ...
            (error) => {
              console.error(
                'Erreur lors de la suppression de cette article',
                error
              );
              this.toastrService.error(
                'Erreur lors de la suppression de ce commentaire'
              );
            }
          );
        } else {
  ...
  modifierAricle() {
    ...
    this.notificationService
      .confirmAlert('voulez-vous vraiment modifier cet article')
      .then((confirmed) => {
        if (confirmed) {
          this.articleService
            .updateArticle(formData, this.elementSelectionner)
            .subscribe(
              (response: any) => {
              ...
              (error) => {
                console.error(
                  'Erreur lors de la modification de cette article',
                  error
                );
                this.toastrService.error(
                  'Erreur lors de la modification de cette article'

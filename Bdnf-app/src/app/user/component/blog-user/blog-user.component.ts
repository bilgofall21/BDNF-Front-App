...
export class BlogUserComponent implements OnInit {
  ...
  getArticlesPage(): any[] {
    ...
    this.allArticleDatatrouve = this.allArticleData.filter(
      (service: { titre: string; contenue: string }) =>
        service.titre
          .toLowerCase()
  ...
  get pages(): number[] {
    ...
    const totalPages = this.allArticleData
      ? Math.ceil(this.allArticleData.length / this.articlesParPage)
      : 0;
